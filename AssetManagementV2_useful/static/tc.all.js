...
//点击资源个数弹出资源信息
function clickdetailinfo(obj,stationtype,p){
    isedit = true;
    var stationid = $(p).find('td').eq(0).text().trim();//修改
    $.ajax({type:'GET',url:'/clickdetailinfo',data:{
        'stationid':stationid,'stationtype':stationtype}, async:false,success:function(data){
        if(data == null){
            $("<div id='tempmsg' style='z_index:10000;'><p>未查询到资源/资产详细信息。</p></div>").appendTo(obj).fadeOut("slow");
            window.setTimeout("$('#tempmsg').remove()",1000);
        }
        else{
            for(var i =0; i<data.length;i++) {
                var row = document.createElement("tr"); //创建行
                row.setAttribute('id', 'inforow'+i.toString());
...
function addinfo(obj) {
    ...
    isadd = true;
    addcount +=1;
    var $tdnew = $("#inforow0").clone();       //增加一行,克隆第一个对象
...
function detailinfo(obj, i){
    var p = obj.parentNode.parentNode.parentNode;
    var row = p.rowIndex + 1;
    if (obj.text == '0'){
        $("<div id='tempmsg' style='z_index:10000;'><p>无资源/资产。</p></div>").appendTo(obj).fadeOut("slow");
            window.setTimeout("$('#tempmsg').remove()",1000);
            return 0;
    }
    else{
        ...
        clickdetailinfo(obj,stationtype,p);
    }
}